...
For the global-float shot, the measured chart now opens automatically on the right. Use **Top-down** or **Angled view** above the scene, and **Temperature / Salinity** in the profile card. No separate “Open selected float profile” step is needed. The blue water block represents geographic/depth context, not a gridded temperature field.
