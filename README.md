...
- Real historical Argo profiles with source, WMO/cycle, timestamps and QC provenance across the Indian, Atlantic, and Pacific oceans.
- Validated CSV/TSV observation uploads for supported scalar profiles and instrument labels.
- Triangulated local isosurfaces, selectable variable/Viridis/thermal palettes, linear/log color scales and continuous depth exaggeration.
...
```mermaid
flowchart LR
    A[Historical or uploaded NetCDF] --> B[xarray and NumPy adapter]
...
| `POST /api/datasets/real-argo` | Restore historical ocean fields and global Argo catalogue |
| `POST /api/observations/upload` | Validate and load observation CSV/TSV |
...
- The historical model subset covers the Indian Ocean; global Argo profiles outside it remain profile-only.
- Chlorophyll is a February monthly satellite composite at the surface only.
- The isosurface display is a triangulated sampled-grid surface; missing cells remain open.
...
docs/                     Presentation, validation, and presentation documentation
...
- [docs/WALKTHROUGH.md](docs/WALKTHROUGH.md): recording walkthrough and recovery steps.
- [docs/SUBMISSION_READINESS.md](docs/SUBMISSION_READINESS.md): current requirement coverage and limits.
